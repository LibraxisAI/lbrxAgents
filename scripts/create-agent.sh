...
cat > "$(dirname "$0")/../cards/${AGENT_NAME}Card.json" <<EOL
{
  "name": "${AGENT_NAME}",
  "version": "1.0.0",
  "id": "${AGENT_UUID}",
...
sed "s/TWÓJ-UUID-TUTAJ/${AGENT_UUID}/g; s/NazwaAgenta/${AGENT_NAME}/g; s/Opis twojego agenta i jego roli/${AGENT_DESCRIPTION}/g" "$(dirname "$0")/../templates/agent-template.js" > "$(dirname "$0")/../examples/agents/${AGENT_NAME}-agent.js"

# Opublikuj możliwości
node -e "const api = require('../src/agent-api.js'); api.publishCapabilities('../cards/${AGENT_NAME}Card.json');"
